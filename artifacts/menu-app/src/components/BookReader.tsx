...
const SAMPLE_CONTENT = [
  ...
  "Frisaba la edad de nuestro hidalgo con los cincuenta años; era de complexión recia, seco de carnes, enjuto de rostro, gran madrugador y amigo de la caza.\n\nQuieren decir que tenía el sobrenombre de Quijada, o Quesada, que en esto hay alguna diferencia en los autores que de este caso escriben; aunque, por conjeturas verosímiles, se deja entender que se llamaba Quejana.",
  ...
  "Y aun la administración de su hacienda; y llegó a tanto su curiosidad y desatino en esto, que vendió muchas hanegas de tierra de sembradura para comprar libros de caballerías en que leer, y así, llevó a su casa todos cuantos pudo haber dellos.\n\nCapítulo 2\n\nY con estas razones perdía el pobre caballero el juicio, y desvelábase por entenderlas y desentrañarles el sentido, que no se lo sacara ni las entendiera el mesmo Aristóteles.",
];
...
export default function BookReader() {
  ...
  const { data: progress } = useQuery({
    queryKey: ["reading_progress", id, user?.id],
    queryFn: async () => {
      if (!user) return null;
  ...
  const saveProgress = useMutation({
    mutationFn: async (percent: number) => {
      if (!user || !id) return;
      const status = percent >= 100 ? "completed" : percent > 0 ? "reading" : "want_to_read";
      if (progress) {
        await supabase
          .from("reading_progress")
          .update({ progress_percent: percent, status })
          .eq("user_id", user.id)
          .eq("book_id", id);
      } else {
        await supabase
          .from("reading_progress")
          .insert({ user_id: user.id, book_id: id, progress_percent: percent, status });
      }
    },
  });
  ...
  useEffect(() => {
    if (user && id) {
      saveProgress.mutate(progressPercent);
    }
  }, [currentPage]);
    ...
    <div className={`fixed inset-0 z-[200] ${t.bg} ${t.text} flex flex-col transition-colors duration-300`}>
      ...
      >
        ...
        <div className="text-center flex-1 min-w-0 px-4">
          <p className="text-xs font-semibold truncate">{book?.title || "Cargando..."}</p>
